...
router.get('/cart-info', async (req, res) => {
  ...
  if (!variantIds || typeof variantIds !== 'string') return res.json([]);
  const ids = [...new Set(variantIds.split(',').map((id) => parseInt(id, 10)).filter(Number.isInteger))];
...
router.get('/:slug/comments', async (req, res) => {
  const [p] = await db.query('SELECT id FROM products WHERE slug = ?', [req.params.slug]);
...
router.post('/:slug/comments', auth, async (req, res) => {
  ...
  const [p] = await db.query('SELECT id FROM products WHERE slug = ?', [req.params.slug]);
  if (!p?.length) return res.status(404).json({});
  const productId = p[0].id;
  const r = rating != null ? Math.min(5, Math.max(1, parseInt(rating))) : null;
  await db.query('INSERT INTO product_comments (product_id, user_id, content, rating) VALUES (?, ?, ?, ?)', [productId, req.user.id, content.trim(), r]);
  const [rows] = await db.query(
    `SELECT pc.id, pc.content, pc.rating, pc.created_at, u.full_name, u.avatar
  ...
  const c = rows?.[0];
  res.status(201).json(c ? { ...c, user_avatar: c.avatar ? '/api/uploads/avatars/' + c.avatar : null } : {});
});
...
router.get('/:slug', async (req, res) => {
  ...
  if (!productRows?.length) return res.status(404).json({});
